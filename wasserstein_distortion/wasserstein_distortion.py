# Copyright 2025 Yueyu Hu, Jona Balle.
#
...
class LowpassFilter2D(nn.Module):
    def __init__(self):
        super(LowpassFilter2D, self).__init__()
    ...
    def forward(self, x, stride=1):
        kernel = self.kernel.expand((x.shape[1], 1, -1, -1)) # type: ignore
        x = nn.functional.conv2d(x, kernel, stride=stride, padding=1, groups=x.shape[1])
        return x

class MultiLevelStats(nn.Module):
    def __init__(self, pyramid_levels=4):
        super(MultiLevelStats, self).__init__()
...
class WassersteinDistortionFeature(nn.Module):
    '''Calculate the Wasserstein distortion between two feature maps.'''
    def __init__(self, pyramid_levels: int = 5):
        super(WassersteinDistortionFeature, self).__init__()
        self.multi_level_stats = MultiLevelStats(pyramid_levels)
        self.pyramid_levels = pyramid_levels
        self.lowpass = LowpassFilter2D()
    ...
    def forward(self,
                ...
                log2_sigma: tuple[float, ...]) -> torch.Tensor:
        '''Calculate the Wasserstein distortion between two feature maps given
        the log2_sigma map.'''
        wasserstein_dist = 0
        mean_pyr_a, var_pyr_a = self.multi_level_stats(features_a)
        mean_pyr_b, var_pyr_b = self.multi_level_stats(features_b)
        # mse_mu_sum, mse_scale_sum = 0, 0
        ...
        for i, wd_map in enumerate(wd_maps):
            weights_i = F.relu(1 - torch.abs(log2_sigma - i)) # type: ignore
            if i > 0:
                log2_sigma = self.lowpass(log2_sigma, stride=2)
            wasserstein_dist += (weights_i * wd_map).mean()
        return wasserstein_dist # type: ignore

class MultiscaleTruncatedVGG(nn.Module):
    '''
    ...
    '''
    def __init__(self, requires_grad=False, pretrained=True, truncate_slice=5,
        ...
        '''
        super(MultiscaleTruncatedVGG, self).__init__()
        ...
        assert truncate_slice <= self.N_slices, f"Truncate slice {truncate_slice} is larger than {self.N_slices}"
        assert truncate_slice >= 1, f"At least one slice is required, but got {truncate_slice}"

        for x in range(4):
            self.slice1.add_module(str(x), vgg_pretrained_features[x]) # type: ignore
        if self.truncate_slice >= 2:
            for x in range(4, 9):
                if replace_with_avg_pooling and isinstance(vgg_pretrained_features[x], nn.MaxPool2d): # type: ignore
                    self.slice2.add_module(str(x), nn.AvgPool2d(kernel_size=2, stride=2))
                else:
                    self.slice2.add_module(str(x), vgg_pretrained_features[x]) # type: ignore
        if self.truncate_slice >= 3:
            for x in range(9, 16):
                if replace_with_avg_pooling and isinstance(vgg_pretrained_features[x], nn.MaxPool2d): # type: ignore
                    self.slice3.add_module(str(x), nn.AvgPool2d(kernel_size=2, stride=2))
                else:
                    self.slice3.add_module(str(x), vgg_pretrained_features[x]) # type: ignore
        if self.truncate_slice >= 4:
            for x in range(16, 23):
                if replace_with_avg_pooling and isinstance(vgg_pretrained_features[x], nn.MaxPool2d): # type: ignore
                    self.slice4.add_module(str(x), nn.AvgPool2d(kernel_size=2, stride=2))
                else:
                    self.slice4.add_module(str(x), vgg_pretrained_features[x]) # type: ignore
        if self.truncate_slice >= 5:
            for x in range(23, 30):
                if replace_with_avg_pooling and isinstance(vgg_pretrained_features[x], nn.MaxPool2d): # type: ignore
                    self.slice5.add_module(str(x), nn.AvgPool2d(kernel_size=2, stride=2))
                else:
                    self.slice5.add_module(str(x), vgg_pretrained_features[x]) # type: ignore
    ...
    def forward(self, X: torch.Tensor, num_scales: int = 3
                ) -> list[torch.Tensor]:
        """
        Forward pass through the truncated VGG network.
        Args:
            X (torch.Tensor): Input image tensor of shape (N, 3, H, W). Assumed
            to be RGB and normalized to [0, 1].
        Returns:
            Dict[int, torch.Tensor]: A dictionary where keys are
...
class WassersteinDistortion(nn.Module):
    '''Calculate the Wasserstein distortion between two images.'''
    def __init__(self, lpips_net: Literal['vgg'] = 'vgg',
                 pyramid_levels: int = 5,
                 grayscale: bool = False,
                 normalize_center_to_zero: bool = False):
        super(WassersteinDistortion, self).__init__()
        self.wasserstein_distortion_feature = WassersteinDistortionFeature(pyramid_levels)
        self.grayscale = grayscale
        self.normalize_center_to_zero = normalize_center_to_zero
        assert lpips_net == 'vgg', "Currently only VGG is supported."
        if lpips_net == 'vgg':
            truncate_slice = 5
            self.feature_backbone = MultiscaleTruncatedVGG(
                requires_grad=False,
                pretrained=True,
    ...
    def forward(self,
                pred: torch.Tensor, gt: torch.Tensor, log2_sigma: torch.Tensor,
                num_scales: int = 3) -> torch.Tensor:
        if self.grayscale:
        ...
        if self.normalize_center_to_zero:
            pred = pred * 2 - 1
            gt = gt * 2 - 1
        assert pred.shape == gt.shape, f"Predicted and ground truth images must have the same shape, but got {pred.shape} and {gt.shape}"
        ...
        for i in range(len(feats_pred)):
            ls = F.interpolate(
                log2_sigma, size=feats_gt[i].shape[-2:],
                mode="bilinear", antialias=True)
            # Rescale sigma to match the feature arrays. For example, if a
            # feature array has a very low spatial resolution, we make sigma
            # correspondingly smaller, because each element in the feature
            # array covers a larger portion of the image. Since we are in log
            # space, we subtract the log of the size ratio and then cap at zero.
            log_ratio_h = np.log2(
                log2_sigma.shape[-2] / feats_gt[i].shape[-2])
            log_ratio_w = np.log2(
                log2_sigma.shape[-1] / feats_gt[i].shape[-1])
            mean_log_ratio = (log_ratio_h + log_ratio_w) / 2
            ls = F.relu(ls - mean_log_ratio)
            wasserstein_dist += self.wasserstein_distortion_feature(
                feats_pred[i], feats_gt[i], ls)
        return wasserstein_dist # type: ignore
